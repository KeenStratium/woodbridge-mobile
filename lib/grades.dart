...
Future<List> fetchMarkingCodes(schoolLevel) async {
  ...
  var response = await http.post(url,
      body: json.encode({'data': schoolLevel}),
      headers: {
...
Future<List> fetchPsychSkills(userId, schoolLevel) async {
  ...
  var response = await http.post(url,
      body: json.encode({
        'data': {'s_id': userId, 'school_level': schoolLevel}
      }),
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
...
class _GradesState extends State<Grades> {
  ...
  Widget build(BuildContext context) {
    Future buildGrades(userId) async {
      ...
      await fetchGrades(userId).then((result) {
    ...
    Future buildPsychSkills(userId, schoolLevel) async {
      Map<String, List<Widget>> skillWidgets = {'Psych': [], 'Motor': []};

      await fetchPsychSkills(userId, schoolLevel).then((result) {
        List resultGrades = result;

        for (var i = 0; i < resultGrades.length; i++) {
          Map subject = resultGrades[i];
          String cat = subject['skill_cat'];

          skillWidgets[cat].add(GradeCard(
            category: subject['skill_cat'],
            includeAve: false,
            grade: Grade(
    ...
    Future buildMarkingCodes(schoolLevel) async {
      ...
      await fetchMarkingCodes(schoolLevel).then((results) {
        for (int i = 0; i < results.length; i++) {
          Map markingCode = results[i];

          markingCodes.add(Flex(
          ...
          if (i != results.length - 1) {
            markingCodes.add(Padding(
              padding: EdgeInsets.symmetric(vertical: 1.0),
              child: Divider(height: 16.0, color: Colors.grey[400]),
            ));
          }
        }
      });
              ...
              child: ListView(
                children: <Widget>[
                          ...
                          FutureBuilder(
                            ...
                            builder:
                              ...
                              if (snapshot.connectionState ==
                                ...
                                return Column(
                                  children:
                                      snapshot.data ?? <Widget>[Container()],
                                );
                              } else {
                  ...
                  Container(
                    ...
                    child: FutureBuilder(
                      future:
                          buildPsychSkills(widget.userId, widget.schoolLevel),
                      builder: (BuildContext context, AsyncSnapshot snapshot) {
                        if (snapshot.connectionState == ConnectionState.done) {
                          List psychSkills = snapshot.data['Psych'];
                          List motorSkills = snapshot.data['Motor'];
                          ...
                          return Column(
                            children: <Widget>[
                              ...
                              Column(
                                children: psychSkills ?? <Widget>[Container()],
                              ),
                              Center(
                                child: Padding(
                                  padding: EdgeInsets.symmetric(vertical: 20.0),
                              ...
                              Column(
                                children: motorSkills ?? <Widget>[Container()],
                              ),
                            ],
                          );
                        } else {
...
class QuarterGrade extends StatelessWidget {
  QuarterGrade({this.label, this.value});
